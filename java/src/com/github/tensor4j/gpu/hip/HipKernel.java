...
/**
 * HIP kernel function handle.
 */
public final class HipKernel implements GpuKernel {

    private final long funcPtr;

    HipKernel(long funcPtr) {
    ...
    public void launch(GpuStream stream, int gridX, int gridY, int gridZ,
        ...
        for (int i = 0; i < buffers.length; i++) {
            ptrs[i] = ((HipBuffer) buffers[i]).devicePointer();
        }
        HipDevice.nLaunchKernel(funcPtr, gridX, gridY, gridZ, blockX, blockY, blockZ,
